-- Swim Tracker - Multi-Swimmer Normalized Database Schema
-- Supports multiple swimmers with proper relational design
...
CREATE TABLE swimmers (
    ...
    date_of_birth DATE,
    current_age INTEGER,
    lsc TEXT, -- Local Swim Committee
    club TEXT, -- Current club/team
...
CREATE INDEX idx_swimmers_club ON swimmers(club);
CREATE INDEX idx_swimmers_lsc ON swimmers(lsc);

-- ============================================
-- 2. COMPETITION RESULTS TABLE (Multi-swimmer)
-- ============================================
CREATE TABLE competition_results (
    id BIGSERIAL PRIMARY KEY,
    swimmer_id BIGINT NOT NULL REFERENCES swimmers(id) ON DELETE CASCADE,
    ...
    -- Meet/Competition details
    course_type TEXT CHECK (course_type IN ('SCY', 'LCM', 'SCM')),
    distance TEXT,
    stroke TEXT,

    -- Official data
    time_standard TEXT, -- Actual standard awarded (BB, B, A, etc.)
    points INTEGER,
    lsc TEXT, -- LSC where meet was held
    team TEXT, -- Team swum for at this meet

    -- Swimmer age at time of swim
    age INTEGER,
...
CREATE INDEX idx_competition_results_swimmer ON competition_results(swimmer_id);
...
CREATE INDEX idx_competition_results_time ON competition_results(swimmer_id, event_name, time_seconds);

-- ============================================
-- 3. TIME STANDARDS REFERENCE TABLE
-- ============================================
...
-- Insert USA Swimming standards for age 10 SCY
...
-- Insert USA Swimming standards for age 10 LCM
INSERT INTO time_standards (event_name, age_group, aaaa_time, aaa_time, aa_time, a_time, bb_time, b_time) VALUES
('50 FR LCM', '10', 31.69, 33.39, 35.29, 37.99, 40.49, 44.49),
('100 FR LCM', '10', 69.59, 73.29, 77.49, 83.59, 89.39, 98.49),
('50 BK LCM', '10', 37.49, 39.49, 41.79, 44.89, 47.99, 52.79),
('100 BK LCM', '10', 81.59, 86.29, 91.39, 99.79, 107.49, 119.39),
('50 BR LCM', '10', 41.79, 44.09, 46.69, 50.19, 53.59, 58.99),
('100 BR LCM', '10', 92.39, 97.99, 104.09, 113.29, 121.79, 136.69)
ON CONFLICT (event_name, age_group) DO NOTHING;

-- ============================================
-- 4. PRACTICE SESSIONS TABLE (Multi-swimmer)
-- ============================================
...
CREATE INDEX idx_practice_sessions_swimmer ON practice_sessions(swimmer_id);
CREATE INDEX idx_practice_sessions_date ON practice_sessions(practice_date DESC);

-- ============================================
-- 5. IMPROVEMENT GOALS TABLE (Multi-swimmer)
-- ============================================
...
CREATE INDEX idx_improvement_goals_swimmer ON improvement_goals(swimmer_id);
...
-- ============================================
-- 6. TEAM PROGRESSION TABLE (Multi-swimmer)
-- ============================================
...
CREATE INDEX idx_team_progression_swimmer ON team_progression(swimmer_id);
...
-- ============================================
-- VIEWS (Calculated on demand, per swimmer)
-- ============================================

-- View: Personal Bests (per swimmer)
CREATE OR REPLACE VIEW personal_bests AS
SELECT DISTINCT ON (swimmer_id, event_name)
    swimmer_id,
    event_name,
...
ORDER BY swimmer_id, event_name, time_seconds ASC, event_date DESC;

-- View: Latest Times Per Event (per swimmer)
...
SELECT DISTINCT ON (swimmer_id, event_name)
    ...
    lsc
FROM competition_results
ORDER BY swimmer_id, event_name, event_date DESC, id DESC;

-- View: Competition Results with Standards (per swimmer)
...
SELECT
    ...
    -- Calculated fields
    get_current_standard(cr.event_name, cr.time_seconds, cr.age::TEXT) as calculated_standard,
    (SELECT next_level FROM get_next_standard_info(cr.event_name, cr.time_seconds, cr.age::TEXT)) as next_standard,
    (SELECT target_time FROM get_next_standard_info(cr.event_name, cr.time_seconds, cr.age::TEXT)) as next_target_seconds,
    format_time((SELECT target_time FROM get_next_standard_info(cr.event_name, cr.time_seconds, cr.age::TEXT))) as next_target_formatted,
    (SELECT gap_seconds FROM get_next_standard_info(cr.event_name, cr.time_seconds, cr.age::TEXT)) as gap_seconds,
    (SELECT improvement_pct FROM get_next_standard_info(cr.event_name, cr.time_seconds, cr.age::TEXT)) as improvement_needed_pct,
...
-- View: Progress Report (per swimmer)
...
SELECT
    ...
    -- Current performance
    get_current_standard(lt.event_name, lt.time_seconds, '10') as current_standard,

    -- Next goal
    (SELECT next_level FROM get_next_standard_info(lt.event_name, lt.time_seconds, '10')) as next_standard,
    format_time((SELECT target_time FROM get_next_standard_info(lt.event_name, lt.time_seconds, '10'))) as next_target_time,
    (SELECT gap_seconds FROM get_next_standard_info(lt.event_name, lt.time_seconds, '10')) as gap_seconds,
    (SELECT improvement_pct FROM get_next_standard_info(lt.event_name, lt.time_seconds, '10')) as improvement_pct,
...
LEFT JOIN personal_bests pb ON lt.swimmer_id = pb.swimmer_id AND lt.event_name = pb.event_name
ORDER BY lt.swimmer_id, lt.event_name;
...
COMMENT ON TABLE swimmers IS 'Master table of all swimmers being tracked';
...
COMMENT ON VIEW personal_bests IS 'Derived view: Best time ever for each event per swimmer';
COMMENT ON VIEW latest_times_per_event IS 'Derived view: Most recent swim for each event per swimmer';
COMMENT ON VIEW competition_results_with_standards IS 'Derived view: All results with calculated standards and gaps';
COMMENT ON VIEW progress_report IS 'Derived view: Clean progress report with no duplicate columns per swimmer';
